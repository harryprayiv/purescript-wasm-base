-- | WasmBase `Wasm.Int64Array` (ADR 0026 sibling of `Wasm.Array`): a first-order,
...
-- | — `Wasm.Int64Array.*`); the accompanying `Wasm/Int64Array.js` provides a
-- | `BigInt64Array`-backed version for stock `purs` / `purs-backend-es`.
module Wasm.Int64Array
  ( Int64Array
  , length
  , unsafeIndex
  , unsafeNew
...
foreign import data Int64Array :: Type

-- | The lane count. On wasm: `Int64ArrayLength` (`array.len`).
foreign import length :: Int64Array -> Int

-- | The raw i64 lane at index `i`, **unchecked** (out of range traps). On wasm:
-- | `Int64ArrayIndex` (`array.get` of an `i64`, no box).
foreign import unsafeIndex :: Int64Array -> Int -> Int64
...
-- | immediately). On wasm: `Int64ArrayNew`.
foreign import unsafeNew :: Int -> Int64Array
...
-- | an effect). Unchecked. On wasm: `Int64ArraySet` (`array.set`, then yields the array).
foreign import unsafeSet :: Int64Array -> Int -> Int64 -> Int64Array